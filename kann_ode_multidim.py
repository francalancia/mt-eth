...
import tqdm
import parameters_ode_multidim
import pandas as pd
import numpy as np
import os
...
def model_evaluation(n_samples,linearspacing, x_min, x_max,y0,step_loc_sol, log_fct,k, model_1, model_2):
    epoch_idx = 0
    # Create Output
    if linearspacing:
    ...
    for sample in range(n_samples):
        ...
        y_hat_1 = y0 + x * model_1(model_input,epoch_idx,sample)
        y_hat_2 = y0 + x * model_2(model_input,epoch_idx,sample)
        dydy_hat_1= torch.autograd.grad(
                    y_hat_1, x, torch.ones_like(x)
                )[0]
        dydy_hat_2= torch.autograd.grad(
            y_hat_2, x, torch.ones_like(x)
        )[0]
        y_hat = (y_hat_1 + y_hat_2) / 2
        dydy_hat = (dydy_hat_1 + dydy_hat_2) / 2
...
def main():
    ...
    # Model Parameters
    n_width = parameters_ode_multidim.n_width
    n_order = parameters_ode_multidim.n_order
    n_samples = parameters_ode_multidim.n_samples
    n_epochs = parameters_ode_multidim.n_epochs
    spacing = parameters_ode_multidim.spacing
    linearspacing = parameters_ode_multidim.linearspacing
    
    # ODE Settings
    x_min = parameters_ode_multidim.x_min
    x_max = parameters_ode_multidim.x_max
    y0 = parameters_ode_multidim.y0
    step_loc = parameters_ode_multidim.step_loc
    step_loc_add = parameters_ode_multidim.step_loc_add
    step_loc_sol = parameters_ode_multidim.step_loc_sol
    ndim_in = parameters_ode_multidim.ndim_in
    k = parameters_ode_multidim.k
    log_fct = parameters_ode_multidim.log_fct
    
    # Solving Methods
    autodiff = parameters_ode_multidim.autodiff
    
    # Data Saving and Plot Options
    save = parameters_ode_multidim.save
    show = parameters_ode_multidim.show
    enable_animation = parameters_ode_multidim.enable_animation
    saveloc = parameters_ode_multidim.saveloc
    anim_intvl = parameters_ode_multidim.animation_interval
    
    # How many different Parametrized versions 
    Tot_runs = parameters_ode_multidim.Tot_runs
    ...
    const_i = torch.zeros_like(x_i, requires_grad=True)
    counter = 0 
            ...
            for batch in range(Tot_runs):
                ...
                for sample in range(n_samples):    
                    ...
                    if autodiff is True:
                        ...
                        y_1 = y0 + x*(model_1(model_input,epoch_idx,sample))
                        dy_1dx = torch.autograd.grad(
                            y_1, x, torch.ones_like(x), create_graph=True, materialize_grads=True
                        )[0]
                        y_2 = y0 + x*(model_2(model_input,epoch_idx,sample))
                        dy_2dx = torch.autograd.grad(
                            y_2, x, torch.ones_like(x), create_graph=True, materialize_grads=True
                        )[0]
                        residual = (((dy_1dx + y_1)*0.5)+((dy_2dx + y_2)*0.5) - j)
                    else:
                        with torch.no_grad():
                    ...
                    if autodiff is True:
                        g_lst_1 = torch.autograd.grad(
                            outputs=residual,
                            inputs=model_1.parameters(),
                            create_graph=True, retain_graph=True
                        )
                        g_lst_2 = torch.autograd.grad(
                            outputs=residual,
                            inputs=model_2.parameters(),
                            create_graph=True, retain_graph=True
                        )
                        if ndim_in > 1:
                            g_lst1_hstack = torch.cat(torch.unbind(g_lst_1[0], dim=-1), dim=-1)
                            g_lst2_hstack = torch.cat(torch.unbind(g_lst_2[0], dim=-1), dim=-1)
                            g_hstack_1 = torch.hstack([g_lst1_hstack, g_lst_1[1]])
                            g_hstack_2 = torch.hstack([g_lst2_hstack, g_lst_2[1]])
                            norm_1 = torch.linalg.norm(g_hstack_1)**2
                            norm_2 = torch.linalg.norm(g_hstack_2)**2
                        else:
                            norm_1 = torch.linalg.norm(torch.hstack(g_lst_1))**2
                            norm_2 = torch.linalg.norm(torch.hstack(g_lst_2))**2
                    else:
                    ...
                    # Kaczmarz update
                    for p, g in zip(model_1.parameters(), g_lst_1):
                        update = (residual / norm_1) * torch.squeeze(g)
                        p.data -= update
                    for p, g in zip(model_2.parameters(), g_lst_2):
                        update = (residual / norm_2) * torch.squeeze(g)
                        p.data -= update
                ...
                del loss, dy_1dx, dy_2dx, residual
                        ...
                        if epoch_idx % anim_intvl == 0:
                            with torch.no_grad():
                                ...
                                for sampleeval in range(n_samples):
                                    ...
                                    y_hat_intermediary_1 = y0+x*model_1(model_input_i,epoch_idx,sampleeval)
                                    y_hat_intermediary_2 = y0+x*model_1(model_input_i,epoch_idx,sampleeval)
    ...
    y_hatvec, dydy_hatvec,y_i_sol = model_evaluation(n_samples,linearspacing, x_min, x_max,y0,step_loc_sol, log_fct,k, model_1, model_2)
